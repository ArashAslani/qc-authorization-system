...
public class PositionHierarchyService
{
        ...
        IReadOnlyCollection<Position> allPositions)
    {
        ...
        if (newParent.CompanyUnitId != position.CompanyUnitId)
        {
            throw new OrganizationDomainException(
                $"Position {position.Id} (company {position.CompanyUnitId}) cannot have parent {newParent.Id} (company {newParent.CompanyUnitId}).");
        }
